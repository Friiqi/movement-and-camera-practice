...
public class PlayerController : MonoBehaviour 
{
	...
	private float toggleMoveCoolodwn = 0.5f;
	...
	private void FixedUpdate ()
	{
		...
		if (isGrounded) 
		{
			...
			{
				currentCooldown = toggleMoveCoolodwn;
				isToggleMove = !isToggleMove;
			}
			//we have taken control of our character and therefore toggle move is no longer valid.
			if(isToggleMove && (Input.GetAxis("Vertical") != 0 || Input.GetButton("Jump") || Input.GetMouseButton(0) && Input.GetMouseButton(1)))
				{
					isToggleMove = false;
				}
			...
			if( Input.GetMouseButton(0) && Input.GetAxis("Horizontal") != 0 && Input.GetAxis("Vertical") != 0)
				...
				currentSpeedModifier = Input.GetAxis("Vertical") < 0 || Input.GetMouseButton(1) && Input.GetAxis("Horizontal") != 0 || Input.GetAxis("Strafing") != 0 
				? speedModifier :1;
