...
public class CameraController2 : MonoBehaviour {
	...
	private void LateUpdate()
	{
		...
		if (Input.GetMouseButton (0) || Input.GetMouseButton (1)) 
		{
			...
			yRotation = ClampAngle (yRotation, verticalMin, verticalMax);
			...
			RaycastHit hit;
			if (Physics.Linecast (target.position, position, out hit, collisionLayerMask)) 
			{
			...
			correctDistance = Mathf.Clamp (currentDistance, minZoom, maxZoom);
	...
	private void RotateBehindTarget()
	{
		...
		float currentAngle = target.eulerAngles.y;
